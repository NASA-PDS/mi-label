...
/**
 * The purpose of this class is to dequote values retrieved from a JSON tree. For reasons unknown
 * (that make no sense), TextNode includes the quotes for toString() (folks: quotes are for
 * representing it IN text, NOT part of the value!!!). But asText() does not return the quotes. So
 * all this is to simply intercept calls to TextNode's to return the unquoted string. Note that we
 * don't actually directly see the TextNode but rather the enclosing node which is indexed at the
 * very end.
 */
...
public class NoQuoteUberspector extends UberspectImpl {
  ...
  public VelMethod getMethod(Object obj, String method, Object[] args, Info i) throws Exception {
    if (obj == null)
      return null;
  ...
  /**
   * The infrastructure wants a class implementing an interface, I guess to make it easier to cache?
   */
  ...
  protected class NoQuoteVelPropertyGetImpl implements VelPropertyGet {
    ...
    /**
     * Does the actual work. I am honestly not sure of the difference between the passed-in o and
     * saved _obj - they seem to be the same.
     */
  ...
  protected class NoQuoteVelMethodGetImpl implements VelMethod {
    ...
    /**
     * Does the actual work. I am honestly not sure of the difference between the passed-in o and
     * saved _obj - but in this case o worked but obj did not.
     */
